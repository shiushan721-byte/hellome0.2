...
export default function CanvasResultView({ type, title, description, artifacts }: Props) {
  ...
  const imageArtifactUrl = (type === 'image' && artifacts && artifacts.length > 0) ? artifacts[0].url : null;
  const textArtifactContent = ((type === 'document' || type === 'report' || type === 'script') && artifacts && artifacts.length > 0) ? artifacts[0].content || artifacts[0].url : null;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-700">
      <div className="w-full max-w-md bg-white rounded-[28px] border border-black/5 shadow-xl overflow-hidden">
        {/* Result Media Area */}
        <div className={`w-full relative flex items-center justify-center group overflow-hidden ${textArtifactContent ? 'bg-[#FCFCFD] border-b border-black/5 h-[300px]' : 'bg-[#111214] aspect-[9/16] max-h-[400px]'}`}>
          {videoArtifactUrl ? (
            <video 
            ...
            <img 
              src={imageArtifactUrl} 
              alt="Result" 
              className="w-full h-full object-cover"
            />
          ) : textArtifactContent ? (
            <div className="w-full h-full p-6 overflow-y-auto custom-scrollbar text-[13px] leading-relaxed text-[#1A1A1A] whitespace-pre-wrap">
              {textArtifactContent}
            </div>
          ) : (
            <>
              ...
              {(type === 'document' || type === 'report' || type === 'script') && <FileText className="h-12 w-12 text-black/20" />}
            </>
          )}
        </div>
