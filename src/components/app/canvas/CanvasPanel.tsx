  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-6 right-6 z-50 flex gap-2">
        <button 
          onClick={centerView}
          className="px-4 py-2 bg-white border border-black/10 rounded-full shadow-sm text-[12px] font-medium text-black/60 hover:bg-black/5 transition-colors"
        >
          居中最新版本
        </button>
      </div>
      ...
      <div 
        className="absolute top-0 left-0 origin-top-left transition-transform duration-300 ease-out"
        style={{
      ...
      >
        ...
        {runs.length === 0 && (
          <div className="absolute canvas-node" style={{ transform: 'translate(-50%, -50%)' }}>
            <div className="w-[360px] bg-white rounded-3xl shadow-xl border border-black/5 p-8 text-center flex flex-col items-center">
              <div className="h-20 w-20 bg-[#F5F5F7] rounded-3xl flex items-center justify-center text-4xl mb-4">
                {config.icon}
              </div>
              ...
              <div className="mt-6 flex items-center gap-2 opacity-50 bg-[#F5F5F7] px-4 py-2 rounded-full">
                <Sparkles className="h-4 w-4" />
                <span className="text-[12px] font-medium uppercase tracking-widest">Waiting for Input</span>
              </div>
            </div>
          </div>
        )}
          ...
          return (
            <React.Fragment key={run.taskId}>
              ...
              {index > 0 && (
                <svg className="absolute overflow-visible pointer-events-none" style={{ left: '-200px', top: `${runYOffset - 350}px` }}>
                  <path 
                    ...
                    strokeWidth="3" 
                  />
                  <rect x="-40" y="280" width="80" height="24" rx="12" fill="white" stroke="rgba(0,0,0,0.1)" />
                  <text x="0" y="296" fontSize="10" fill="rgba(0,0,0,0.5)" textAnchor="middle" fontWeight="bold">修改 v{index + 1}</text>
                </svg>
              )}
              ...
              <div className="absolute canvas-node transition-all duration-500" style={{ transform: `translate(-400px, ${-250 + runYOffset}px)` }}>
                <div className={`w-[320px] bg-white rounded-[24px] shadow-xl border border-black/5 overflow-hidden flex flex-col h-[500px] ${!isLatest ? 'opacity-70 scale-95 origin-top-left' : ''}`}>
                  <div className="px-5 py-4 border-b border-black/[0.05] bg-[#FCFCFD] shrink-0 flex justify-between items-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#EAF6F4] text-[#0F766E] text-[11px] font-bold tracking-wider uppercase">
                      <Activity className="h-3.5 w-3.5" />
                      Execution Logs v{index + 1}
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4 relative">
                    <div className="absolute left-[39px] top-5 bottom-5 w-px bg-gradient-to-b from-transparent via-black/10 to-transparent pointer-events-none" />
                    {run.events.length === 0 ? (
                      <div className="text-center py-10 text-sm text-black/40 animate-pulse">
                        等待底层日志回传...
                      </div>
                    ) : (
                      run.events.map((event, i) => (
                        <div key={event.id || i} className="relative z-10 flex items-start gap-3">
                          <div className="mt-0.5 flex shrink-0 h-8 w-8 items-center justify-center rounded-full bg-white border border-black/10 shadow-sm text-black/40">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#0F766E]" />
                          </div>
                          <div className="flex-1 bg-[#FCFCFD] rounded-xl border border-black/[0.05] p-3 shadow-sm">
                            <p className="text-[13px] text-[#1A1A1A] font-medium leading-relaxed">{event.message}</p>
                            <p className="text-[10px] text-black/30 mt-1.5">{new Date(event.createdAt).toLocaleTimeString()} · {event.type || 'Sys'}</p>
                          </div>
                        </div>
                      ))
                    )}
              ...
              <svg className="absolute overflow-visible pointer-events-none" style={{ left: '-80px', top: `${runYOffset}px` }}>
                <path 
                  ...
                  strokeWidth="2" 
                  strokeDasharray="6 6"
                  className={run.status === 'running' ? 'animate-[dash_2s_linear_infinite]' : ''}
                />
              </svg>
